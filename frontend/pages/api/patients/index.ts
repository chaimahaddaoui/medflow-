...
async function sendWelcomeEmail(
  ...
  // Génère la liste des médecins en HTML
  const doctorsList = doctors.map(d => 
    `<li><strong>${d.prenom} ${d.nom}</strong> - ${d.specialite || 'Spécialité non spécifiée'}</li>`
  ).join('');
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 20px;">
      <div style="background-color: white; border-radius: 8px; padding: 20px; max-width: 600px; margin: 0 auto;">
        ...
        <p>Votre compte patient a été créé avec succès. Vous êtes désormais enregistré pour prendre vos rendez-vous médicaux.</p>
        ...
        <div style="background-color: #f0f8ff; padding: 15px; border-radius: 5px; border-left: 4px solid #0066cc;">
          <p><strong>Clinique :</strong> ${clinicName}</p>
...
export default async function handler(req: any, res: any) {
  if (req.method === 'GET') {
    ...
    const { nom, prenom, email, dateNaissance, telephone, adresse, clinicId } = req.body;
    try {
      // Génère un mot de passe automatiquement
      const tempPassword = Math.random().toString(36).slice(-8);
      const hashedPassword = await bcrypt.hash(tempPassword, 10);

      // Crée le patient avec le mot de passe haché
      const patient = await prisma.patient.create({
        data: {
          nom,
          prenom,
      ...
      // Récupère les infos de la clinique et ses médecins
      let clinicName = '';
      let clinicAddress = '';
      let doctors: any[] = [];
      ...
      res.status(500).json({ error: 'Erreur création', detail });
    }
  } else {
    res.status(405).end();
  }
